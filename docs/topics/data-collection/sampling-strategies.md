...
- **Keyword-based.** Collect every post that matches a list of terms. Misses relevant posts that use other words, and picks up unrelated posts when a term has more than one meaning. One with domain knowledge could come up with a list of terms, or Snowball sampling, described below, is a common data-driven approach to build the list.
...
- **Snowball on the graph.** Start from seed accounts, collect their followers, then their followers' followers. Maps a community, but the number of requests grows by a factor of a thousand at each step, so a boundary needs to be decided before starting.
...
Keyword-based collection is only as good as the keyword list, and the list is rarely complete on the first try. A researcher who starts from `vaccine` will miss posts that only say `vax` or `Pfizer` for a vaccine-focused data collection.
Snowball sampling grows the list from the data itself.
...
5. Repeat until the list saturates, i.e., a round finds very few or no new relevant terms.

The picture behind this is a network.
Each keyword is a node, and two keywords are connected when they appear together in posts.
Snowball sampling is a search over that network, one layer of neighbors per round.
...
Both assumptions typically hold in practice.
But additional validation is always a good idea.
...
- **False positives.** A term that belongs to the topic in one context has other meanings elsewhere. For example, [in the 2022 U.S. midterm collection](https://ojs.aaai.org/index.php/ICWSM/article/view/22205), `vote` matched posts about the American Music Awards and elections in other countries, and `midterm` matched posts about exams.
